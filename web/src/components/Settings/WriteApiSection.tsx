    ...
    <SettingGroup title="API Webhook" description="A simple Write-Only API endpoint for quick integrations (Shortcuts, Scripts, Raycast).">
      <div className="flex flex-col gap-4 w-full">
        <p className="text-sm text-muted-foreground">
          Create a <strong>Personal Access Token</strong> in the section above, and insert it into the URL below. You can send an HTTP POST
          request to this URL to create a memo without needing Authorization headers.
        </p>
        ...
        <div>
          ...
          <pre className="bg-muted p-3 pl-4 rounded-md font-mono text-xs overflow-x-auto whitespace-pre-wrap">{curlExample}</pre>
        </div>
      </div>
    </SettingGroup>
  );
